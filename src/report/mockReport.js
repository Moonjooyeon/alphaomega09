export function localMockReport(subjects, solo) {
  ...
  if (solo) {
    ...
    return {
      ...
      prognosis: {
        phase_1: "평시에는 손등을 보이지 않는 쪽으로 접어 두고, 남이 가까워질 때마다 먼저 의자 위치를 반 뼘 물린다.",
        phase_2: "주기가 가까워지면 말보다 숨을 먼저 아낀다. 괜찮다는 대답 뒤에 남은 컵 가장자리만 오래 식지 않는다.",
        phase_3: "혼자 두면 문 쪽을 등진 채 같은 천을 접었다 편다. 특정 향이 없으면 안정 구역을 완성하지 못한 채 밤을 넘긴다.",
      },
  ...
  return {
    ...
    imprint: {
      ...
      fixation: "불완전 고정",
      stability: 64,
      rationale: "시선은 피하면서도 손목 안쪽의 맥박이 먼저 빨라진다. 잡히기 직전 소매를 내리는 버릇이 반복되어 각인 방향이 선명해졌다.",
      note: "가까이 있으면 손을 감추고, 떨어지면 같은 자리를 자꾸 눌러 확인한다. 확정 전 단계임에도 분리 구간에서 반응이 더 깊게 남는다.",
    },
    imprint_loss: {
    ...
    prognosis: {
      phase_1: "처음 새는 건 말이 아니라 보폭이다. 한쪽이 지나간 복도에 다른 쪽의 발끝이 정확히 반 박자 늦게 겹친다.",
      phase_2: "먼저 건드린 쪽은 가볍게 물러나지만, 역으로 문고리에 남은 체온을 지나치지 못한다.",
      phase_3: "먼저 핑계를 잃는 쪽은 손끝이고, 늦게 무너지는 쪽은 침묵이다. 끝까지 아닌 척한 쪽이 결국 상대의 향이 남은 물건을 치우지 못한다.",
    },
