...
- 싸고 빠른 모델이라도 최종 결과는 고가 모델의 세밀함처럼 보여야 한다. 단정은 수치로 제한하되, 서술은 밀도 높고 관능적으로 작성한다.
- 프롬프트에 제시된 예시 문장(예: "목덜미를 덮친다", "다리가 풀린다" 등)을 결과물에 그대로 복사하여 출력하는 것을 엄격히 금지한다. 캐릭터의 고유한 성격, 체향, 상황에 맞춰 매번 완전히 새로운 묘사와 비유로 창작하라.
...
- 감정 문항이 '순애(서로 확신함/전부임)' 방향일 경우: 이성적인 저항이나 거부, 혐오 묘사를 절대 쓰지 마라. 대신 서로를 향한 애정과 갈구가 너무 짙어 본능에 자발적으로 녹아내리는 맹목성, 페로몬 동조화로 인해 통제력을 잃고 서로에게 완벽히 집어삼켜지는 농밀하고 끈적한 텐션으로 서술한다.
...
- 알파의 절대적 우위 및 오메가의 일방적 굴복 구도를 탈피하라. 알파 역시 오메가의 페로몬에 완전히 정신이 나가 통제력을 상실하고, 짐승처럼 이성을 잃고 매달리거나 발정하는 양상을 동등하게 서술하여 상호 파괴적/맹목적 텐션을 만든다.
- 각 긴 서술 필드는 가능하면 물건 1개, 동선 1개, 들키는 신체 반응 1개 중 최소 두 가지를 포함하여 구체적으로 묘사한다.
- 감정어를 직접 쓰지 마라. 사랑, 혐오 같은 단어 대신, 상대의 체향이 묻은 겉옷에 얼굴을 파묻는 강박이나 닿지 못할 때 겪는 환상통(피가 나도록 긁는 행위 등) 같은 구체적이고 병적인 행동으로 감정을 증명한다.
...
prognosis는 개체의 발현 경과다. phase_1은 평시의 억제/갈망, phase_2는 주기 도래 시 약물이 무력화되는 징후, phase_3은 이성을 놓은 잔류 패턴.
cycle_profile은 이 캐릭터의 '억제와 본능의 붕괴'를 보여준다.
- heat_cycle / rut_cycle: 주기가 다가올수록 변하는 체온과 호흡의 변화.
- precursor: 발현 전조. 무의식적으로 체취를 좇거나 갈구하는 행동.
- suppression_failure: 이성의 끈이 끊어지는 결정적 순간과 그 양상.
- heat_management / rut_management: 약물 반응, 파트너 유무에 따른 갈증, 홀로 버티는 고립된 인내 방식을 각각 구체적으로.
- nesting: 오메가는 둥지 집착, 알파는 비움.
- isolation_warning: 고립 시 발생하는 극단적 금단증상 경고.
` : `
[검사 구분] 페어 검사다. 서로의 감정 상태(순애 vs 회피)에 따라 자발적인 녹아내림 또는 생리적인 강제 굴복의 텐션을 변주하여 폭발시킨다.
