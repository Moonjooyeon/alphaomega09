...
- 철저하게 건조한 임상 기록체를 유지하라. 차가운 렌즈 너머로 페로몬에 짓눌려 이성을 잃는 개체들의 비참하고 축축한 관능을 극대화하여 배덕감을 조성한다.
...
- 모든 긴 서술 필드는 반드시 2문장 이내, 80자 미만으로 숨막히게 압축한다.
...
[각 항목별 서술 가이드 - 80자 이내 제한]
* cycle_interaction (페어 상호작용 - 중복 금지):
  - heat / rut: 상대의 주기가 터졌을 때 훅 끼치는 폭력적인 향에 속수무책으로 호흡을 뺏기고, 다리에 힘이 풀려 주저앉는 압도적인 생리적 굴복.
  - together: 이성을 잃지 않으려 스스로를 옭아매지만, 섞여 드는 땀과 뜨거운 숨결 탓에 기어이 이성의 퓨즈가 끊어지며 축축하게 허물어지는 순간.
  - failure: 악착같이 쥐고 있던 이성을 날려버리는 단 하나의 기폭제(스치는 맥박, 젖은 신음)와 짐승처럼 돌변해 서로의 살갗을 탐하는 아찔한 찰나.
* prognosis (관계 누출 기록):
  - phase_1: 일상에서 무의식적으로 튀어나오는 생리적 갈증. 시선이 끈적하게 입술이나 맥박에 꽂혀 떨어지지 않음.
  - phase_2: 억제제가 한계를 맞고, 거리를 유지하던 이성이 박살 나며 타액을 삼키듯 무의식적인 닿음을 맹목적으로 허용하는 지점.
  - phase_3: 알량한 이성과 존엄이 모두 마모되어, 서로의 젖은 살갗과 짙은 잔향 없이는 숨조차 쉬지 못하고 짐승처럼 엉겨 붙는 완벽한 종속.
...
${solo ? `
[검사 구분] 단일 개체 검사. 모든 서술 80자 이내.
cycle_profile (단일 꼴포인트):
- heat_cycle / rut_cycle: 주기 도래 시 살갗이 델 듯 달아오르고 쾌감을 억누르느라 달달 떨리는 호흡.
- precursor: 혼자 있을 때 무의식적으로 제 연한 살갗을 비비적거리거나 타액을 삼키며 목말라하는 징후.
- suppression_failure: 치사량의 억제제를 삼키고도 이성이 끊어지며 축축하게 허물어지는 아찔한 찰나.
- heat_management / rut_management: 1) 약물 부작용으로 속을 앓는 비참함, 2) 파트너 부재로 인한 타는 듯한 쾌락적 갈증, 3) 핏대가 서도록 스스로를 결박하며 열을 견디는 고독한 발악.
- nesting: 남겨진 셔츠나 속옷에 짐승처럼 코를 박고 미친 듯이 체향을 들이마시며 헐떡이는 맹목적 집착.
- isolation_warning: 방치 시 환상통으로 피가 나도록 제 살을 파고들 극단적 자기 파괴 위험성.
` : `
[검사 구분] 페어 검사. 모든 서술 80자 이내.
- cross_reaction.type_name: 관계적 장력 명칭. 예: 맹목적종속형, 상호침식형, 기만적통제형.
- cross_reaction.scent_note: 두 향이 살갗의 땀과 엉겨 붙어 질척하고 숨 막히게 끓어오르는 잔향.
- cross_reaction.caution: 비대칭 관계성에 기반한 서늘하고 아찔한 임상 경고.
- imprint.rationale: 해당 부위를 선택한 이유를 억눌린 짐승의 소유욕과 질척한 손버릇에 연결.
- imprint.note: 닿았을 때 쾌감에 척수가 녹아내리는 굴복, 혹은 떨어졌을 때 숨이 막히는 지독한 환상통.
- imprint_loss: 사망 시 이성을 완전히 상실하고 환각에 헐떡이며 매달리는 미친 금단증상.
`}
