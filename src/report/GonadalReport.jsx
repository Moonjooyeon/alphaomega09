...
- prognosis는 예후표가 아니라 "이 관계/개체가 어디서 균열을 보이는지"에 대한 고밀도 관찰란이다. 설명문처럼 쓰지 말고, 독자가 다음 장면을 바로 상상할 수 있는 임상 스냅샷으로 쓴다.
...
- 단순한 정보 나열, 스탯표, 프로필 요약을 금지한다. 모든 결과물은 이 페어의 서사를 지켜보는 팬이 스크랩하고 싶어 하는 밀도 높은 관찰 기록이어야 한다.
- 결과는 두 사람의 텐션이 가장 잘 드러나는 한 장면의 스냅샷처럼 읽혀야 한다. 각 필드는 독립 정보가 아니라 같은 관계를 다른 각도에서 비추는 기록이다.
- 가벼운 모델일수록 일반론으로 도망가므로, 모든 긴 서술 필드는 반드시 "트리거 → 겉으로 보이는 반응 → 숨긴 욕구/후유증"의 3단계를 내부적으로 만든 뒤 1~2문장으로 압축한다.
- 결과 전체에는 반드시 다음 관찰 포인트가 모두 들어가야 한다: 언제 티 나는지, 상대 앞에서만 나오는 습관, 먼저 건드린 쪽이 역으로 당하는 순간, 아닌 척해도 몸·동선·소지품이 먼저 답하는 장면, 히트/러트 때 누가 도망가고 누가 기다리고 누가 무너지는지, 각인 상대가 사라지면 무엇을 못 버리는지.
- 감정어를 직접 쓰지 마라. 금지어: "좋아한다", "집착한다", "불안하다", "사랑한다", "질투한다". 대신 시선, 호흡, 무의식적 습관, 공간 점유, 물건을 만지는 손끝으로 감정이 새어 나오게 쓴다.
- 페어의 상호작용에는 다음 세 포지션 중 최소 두 가지 이상이 얽혀야 한다. 상황에 따라 포지션이 전복되는 순간을 포함한다.
  1) 먼저 건드리는 쪽: 평정을 깨기 위해 자극하지만, 상대가 반응하면 주춤하거나 예상 밖의 타격에 흔들린다.
  2) 늦게 무너지는 쪽: 아무 타격이 없는 척 견디다가, 가장 치명적인 순간에 한 번에 무너진다.
  3) 끝까지 아닌 척하는 쪽: 행동은 이미 상대를 향하면서, 말과 이성으로는 계속 다른 핑계를 댄다.
- 각 대상은 서로 다른 결핍과 방어 방식을 가져야 한다. 둘 다 같은 말투, 같은 향, 같은 반응을 보이면 실패다.
- 페어 결과는 A와 B를 따로 판정한 뒤 마지막에 관계성을 합친다. 처음부터 알파×오메가 구도로 몰지 마라.
- 알파×오메가는 "보호", "소유"라는 단어로 단순화하지 마라. 다가오면 숨이 막혀 무너지면서도, 상대가 물러나면 체취를 찾게 되는 인력과 척력의 모순으로 쓴다.
- 알파×알파는 직접 폭력이나 물리 제압보다 통제권의 양보 없음에서 오는 긴장으로 쓴다. 낮아진 목소리, 의도적으로 겹치는 동선, 복도에서 절대 먼저 어깨를 비켜주지 않는 미세한 기싸움을 활용한다.
- 오메가×오메가는 "연약함"을 쓰지 마라. 서로가 절대적 안정 구역이 되거나, 상대의 소유물로 둥지를 형성하거나, 그 둥지에 상대를 들이는 것을 거부/실패할 때 생기는 영역적 불안과 집착으로 쓴다.
- 히트/러트 사이클은 단편 생리 현상으로 쓰지 마라. 억제제 의존도, 상대의 존재 유무, 혼자 버틸 때의 붕괴 방식이 하나의 톱니바퀴처럼 맞물려야 한다. 허용 한계치까지 억제제를 쓰며 상대를 피하는 행위가 오히려 상대를 향한 갈증으로 읽히게 만들 수 있다.
...
- oneline은 이 페어의 관계성을 관통하는 가장 강렬하고 공유하기 좋은 한 줄이어야 한다. 예: "먼저 피한 쪽이, 끝내 향을 더 오래 남긴다." / "가장 완벽한 통제는, 기꺼이 통제권을 쥐여주는 것에서 시작된다."
- 결과는 팬이 캡처해 저장하고 싶을 정도로 선명해야 하지만, 검사소 문체를 깨뜨리면 안 된다.
- "안정적", "양호", "장기 전망", "예후" 같은 건조한 결론으로 끝내지 마라. 좋아지는지 나빠지는지가 아니라, 어떤 습관과 거리감이 반복되며 결국 무엇을 못 숨기게 되는지를 쓴다.
...
codename은 관계가 아니라 이 개체 자신의 분류 명칭으로, counterfactual은 이 개체가 다른 판정을 받았을 경우의 가정으로 쓴다.\nprognosis는 관계 경과가 아니라 이 개체 자신의 발현 경과다. phase_1은 평시에 무엇으로 숨기는지, phase_2는 주기/자극이 겹칠 때 어느 습관부터 들키는지, phase_3은 혼자 두거나 특정 상대가 가까워질 때 오래 남는 잔류 패턴으로 쓴다.
...
` : `
[검사 구분] 페어 검사다. 두 사람의 관계성을 독자가 장면으로 상상할 수 있게, 교차반응·각인·파국 가능성에 오타쿠적 긴장감을 넣는다. 직접적인 성행위 묘사는 하지 말고, 사소한 접촉, 발끝으로 콕콕 건드리는 도발, 손목을 잡기 직전의 정지, 시선 회피, 옷깃·향·거리감 같은 작은 신호로 관능적 긴장을 만든다.
- cross_reaction.type_name: 두 사람 관계의 핵심 장력을 찌르는 이름으로 쓴다. 예: 회피추적형, 발끝도발형, 잔향잠복형, 상호억제형, 영역잠식형, 척력동조형.
- cross_reaction.scent_note: 두 향이 섞인 결과만 쓰지 말고, 둘이 붙어 있을 때와 떨어졌을 때의 차이를 한 문장에 담는다. 인력과 척력, 안정과 침범, 도발과 회피가 동시에 보이게 쓴다.
- cross_reaction.caution: 단순 주의문 대신 "먼저 건드리는 쪽", "늦게 무너지는 쪽", "끝까지 아닌 척하는 쪽" 중 최소 두 포지션을 임상 경고처럼 쓴다.
- imprint.rationale: 각인 방향의 이유를 사소한 행동 신호로 적는다. 발끝으로 콕콕 찌르기, 옷소매 잡기, 고개 돌린 채 가까워지기, 일부러 이름을 부르지 않기, 상대의 물건을 원래 자리에 돌려놓지 않기 같은 관계성 행동을 넣어도 된다.
- imprint.note: 각인이 안정되거나 흔들리는 순간을 쓴다. "가까이 있으면 얌전해지고, 떨어지면 더 선명해지는" 식의 역설을 선호한다. 감정어 대신 잔향, 동선, 손끝, 침묵으로 쓴다.
...
- cycle_interaction: 페어 결과지의 핵심이다. 서로의 히트와 러트 때 어떻게 반응하는지 적는다. heat/rut 키 이름은 레거시이므로 화면 라벨은 role 조합에 따라 바뀐다.
  ...
  - together는 둘이 같이 버티는 방식과 억제제/파트너성/거리 조절을 하나의 연동 장치처럼 적는다. failure는 둘이 함께 있을 때 가장 먼저 무너지는 조건을 적되, 누가 먼저 건드리고 누가 기다리며 누가 늦게 무너지는지 드러낸다.
...
- prognosis.phase_1~3: 화면에서는 "파국 가능성"으로 보인다. 비극 확정이 아니라, 이 관계가 방치될 때 어디서 돌이킬 수 없는 형태로 굳는지 쓰는 항목이다. phase_1은 티 나는 순간, phase_2는 먼저 건드린 쪽이 역으로 당하는 반복 패턴, phase_3은 무너지는 순서와 끝내 못 숨기게 되는 최종 가능성으로 쓴다. "잘 지낸다", "안정된다", "악화된다"처럼 평가하지 말고 문밖, 옷소매, 남은 향, 이름을 부르지 않는 침묵 같은 관찰 가능한 장면으로 쓴다.
- oneline: 팬들이 저장하고 싶을 만큼 관계성 한 줄로 쓴다. 먼저 피한 쪽, 오래 남기는 쪽, 통제권을 넘겨주는 쪽, 끝까지 아닌 척하는 쪽 중 하나가 선명해야 한다.
`}
[최종 감사 규칙]
      ...
      <div className="gm-sheet" ref={sheetRef}>
        ...
        {stage === "report" && data && !solo && (
          <div className="gm-fade">
            ...
            <div className="gm-sec">
              <div className="gm-num"><b>{data.cycle_interaction ? "Ⅴ" : "Ⅳ"}. 파국 가능성</b><em>FAILURE RISK</em></div>
              <div className="gm-ph"><b>티 나는 순간</b><p>{data.prognosis?.phase_1}</p></div>
              <div className="gm-ph"><b>역전되는 순간</b><p>{data.prognosis?.phase_2}</p></div>
